...
func startPoll(cmd *cobra.Command, args []string) error {
    ...
    if resp.StatusCode != http.StatusOK {
        var errRes struct { Error string `json:"error"` }
        if decodeErr := json.NewDecoder(resp.Body).Decode(&errRes); decodeErr == nil && errRes.Error != "" {
            return fmt.Errorf("error starting poll: %s", errRes.Error)
        }
        // fallback to raw body
        raw, _ := ioutil.ReadAll(resp.Body)
        return fmt.Errorf("error starting poll: %s", strings.TrimSpace(string(raw)))
    }
    ...
    fmt.Println("Poll link:", res.PollLink)
    fmt.Println("QR code link:", res.QRLink)
